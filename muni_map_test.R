...
# DEFINE NON-REACTIVE FUNCTIONS
  ...
  getGlobalBinRanges <-
    function(func_datatype_input, func_aggregation_input, func_metric_input) {
...
if (!inherits(muni_map_sf, "sf")) {muni_map_sf <- st_as_sf(muni_map_sf, wkt = "geometry_column")}
...
server <- function(input, output, session) {
  ...
  dynamicPopupContent <-
    function(func_metric_input,func_aggregation_input,func_datatype_input,feature) {
  ...
  createOlistMap <-
    function(data, func_metric_input, func_datatype_input,func_aggregation_input) {
      if (!inherits(data, "sf")) {
      ...
      leaflet(data) %>%
        ...
        addPolygons(
          ...
          color = "transparent",
          fillOpacity = 0.7,
          popup = ~ dynamicPopupContent(selected_metric(), selected_aggregation(), selected_data_type(), .)
        ) %>%
        addLegend(
