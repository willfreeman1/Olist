...
# First calc weighted products for later weight avgs
orders_items_customers_metrics_byzip_geo <- orders_items_customers_metrics_byzip_geo %>%
  mutate(
    weighted_prod_delivery_time = avg_delivery_time * total_orders,
    weighted_prod_expected_delivery_time = avg_expected_delivery_team * total_orders,
    weighted_prod_additional_delivery_time = avg_additional_delivery_time * total_orders,
    weighted_prod_late_delivery_percent = avg_late_delivery_percent * total_orders
    # Add similar calculations for any other weighted averages
  )
...
orders_items_customers_metrics_muni_geo <- orders_items_customers_metrics_byzip_geo %>%
  ...
  summarize(
    ...
    avg_delivery_time = sum(weighted_prod_delivery_time, na.rm = TRUE) / total_orders,
    avg_expected_delivery_time = sum(weighted_prod_expected_delivery_time, na.rm = TRUE) / total_orders,
    avg_additional_delivery_time = sum(weighted_prod_additional_delivery_time, na.rm = TRUE) / total_orders,
    avg_late_delivery_percent = sum(weighted_prod_late_delivery_percent, na.rm = TRUE) / total_orders,
...
leaflet(custmetrics_zip_geo) %>%
    ...
    addPolygons(
        ...
        label = ~paste("Zip Code:", zip_code_prefix,
                       "Total Sales: R$", formatC(total_sales, format = "f", big.mark = ".", decimal.mark = ",", digits = 2)),
    ...
    addLegend(
        ...
        labFormat = labelFormat(transform = function(x) {
            if (is.numeric(x)) {
                formatC(expm1(x), format = "f", big.mark = ".", decimal.mark = ",", digits = 2)
            } else {
                as.character(x)
            }
        }), # Inverse of log1p for legend labels
        opacity = 0.7
    )
...
```{r}
ggplot(custmetrics_zip_geo, aes(x = total_orders)) +
  ...
  theme_minimal() +
  scale_x_continuous(labels = scales::comma)  # Format the x-axis labels without scientific notation

summary(custmetrics_zip$total_repeat_customers)
...
```{r}
custmetrics_state_geo <- custmetrics_state_geo %>%
  st_as_sf(custmetrics_state_geo)

custmetrics_muni_geo <- custmetrics_muni_geo %>%
  st_as_sf(custmetrics_muni_geo)

custmetrics_zip_geo <- custmetrics_zip_geo %>%
  st_as_sf(custmetrics_zip_geo)

getwd()

saveRDS(custmetrics_state_geo,"..//muni_map_test//custmetrics_state_geo.rds")
saveRDS(custmetrics_muni_geo,"..//muni_map_test//custmetrics_muni_geo.rds")
saveRDS(custmetrics_zip_geo,"..//muni_map_test//custmetrics_zip_geo.rds")
