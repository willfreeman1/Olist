...
```{r}
# Since there are many features we'll first check for multicollinearity

library(caret)
library(glmnet)

# Include only numeric variables
numeric_features = full_cust_features %>% select_if(is.numeric)

# Standardize the numeric features

standardized_features = preProcess(numeric_features, method = c("center", "scale"))
normalized_data = predict(standardized_features, numeric_features)

full_cust_features_RFM = full_cust_features %>%
  left_join(final_cust_RFM, by = "customer_unique_id")

final_analysis_df = full_cust_features_RFM %>%
  filter(!is.na(RFM_avg))
